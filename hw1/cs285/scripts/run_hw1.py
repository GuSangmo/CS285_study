...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          ob_batch, ac_batch = replay_buffer.obs, replay_buffer.acs 

          sampled_indices = np.random.permutation(ob_batch.shape[0])[:params['train_batch_size']]

          ob_sampled = ob_batch[sampled_indices]
          ac_sampled = ac_batch[sampled_indices]

          # use the sampled data to train an agent
          train_log = actor.update(ob_sampled, ac_sampled)
