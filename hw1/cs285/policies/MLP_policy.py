...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        chosen_mean = self.mean_net(observation)
    ...
    def update(self, observations, actions):
        ...
        """
        SM) observation: (Batch, ob_dim), action: (Batch, ac_dim) 형태로 들어옴. 
        또한 observations와 actions 모두 ndarray 형태로 들어옴.
        """
        ...
        loss =  (-1) * pred_action_distribution.log_prob(actions).mean()
        loss.backward() 
        self.optimizer.step()
