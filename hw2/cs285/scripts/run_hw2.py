...
def run_training_loop(args):
    ...
    for itr in range(args.n_iter):
        ...
        ## FIXME: 
        ## Example format => observation / image-obs / reward / action / next_observation.
        trajs_dict = {k: [traj[k] for traj in trajs] for k in trajs[0]}

        # TODO: train the agent using the sampled trajectories and the agent's update function
        train_info: dict = {}

        ## FIXME: Check whether this operation should be extended, or one-time manner
        ### SM) Watch out for obs especially
        ### Look this comment later and improve again
        """

        - Obs: 28 trajectory => (27,4), (32,4)
        - Action: 28 trajectory => (27,) , (32,)
        - Reward:(...) 

        """
        train_info['obs'] = trajs_dict['observation']
        train_info['actions'] = trajs_dict['action']
        train_info['rewards'] = trajs_dict['reward']
        train_info['terminals'] = trajs_dict['terminal']
        agent.update(**train_info)
        ...
        if itr % args.scalar_log_freq == 0:
            ...
            for key, value in logs.items():
                ...
                #FIXME) Hot-fix concat values
                if key in ['obs', 'actions', 'rewards', 'terminals'] : continue 
